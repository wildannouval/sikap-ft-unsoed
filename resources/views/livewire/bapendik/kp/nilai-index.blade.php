    ...
    <flux:card class="space-y-4">
        <div class="flex items-center justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-zinc-500">Lihat nilai akhir, unduh BA Scan (upload dospem) & BA DOCX (generator).
                </p>
            </div>
            ...
            <flux:table.rows>
                @foreach ($items as $i => $row)
                    <flux:table.row :key="$row->id">
                        ...
                        <flux:table.cell>
                            @if ($row->grade)
                                <div class="text-sm font-medium">{{ $row->grade->final_score }}
                                    ({{ $row->grade->final_letter }})</div>
                            @else
                        ...
                        <flux:table.cell class="text-sm">
                            @if ($row->grade)
                                Dospem {{ $row->grade->score_dospem }} • PL {{ $row->grade->score_pl }}
                            @else
                                —
                            @endif
                        </flux:table.cell>
                        ...
                        <flux:table.cell>
                            @if ($row->status === 'ba_terbit')
                                <a class="text-sm underline" href="{{ route('bap.kp.seminar.download.ba', $row->id) }}"
                                    target="_blank">
                                    Unduh BA (DOCX)
                                </a>
                            @else
