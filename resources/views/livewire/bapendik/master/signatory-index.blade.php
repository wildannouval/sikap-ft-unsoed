<div class="space-y-6">
    ...
    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <flux:subheading class="text-zinc-600 dark:text-zinc-300">
                Kelola pejabat penandatangan untuk dokumen SPK & Surat Pengantar.
            </flux:subheading>
        </div>
    </div>
        ...
        <div class="lg:col-span-3 space-y-6">
            <flux:card
                ...
                <div
                    ...
                    <h4 class="text-base font-semibold text-stone-900 dark:text-stone-100">Daftar Pejabat</h4>
                ...
                <flux:table :paginate="$this->items">
                    <flux:table.columns>
                        ...
                        <flux:table.column sortable :sorted="$sortBy === 'name'" :direction="$sortDirection"
                            wire:click="sort('name')">Nama Lengkap</flux:table.column>
                        ...
                        @foreach ($this->items as $i => $row)
                            <flux:table.row :key="$row->id">
                                ...
                                <flux:table.cell class="max-w-[200px]">
                                    <span class="truncate">{{ $row->position }}</span>
                                </flux:table.cell>
                ...
                @if ($this->items->isEmpty())
                    <div class="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p class="mt-1 text-sm text-zinc-500">
                            ...
                            @else
                                Belum ada penandatangan yang ditambahkan.
                            @endif
                        </p>
                    </div>
                @endif
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <flux:card
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-sky-900 dark:text-sky-100 text-sm">Info Penandatangan</h3>
                        <ul class="mt-3 text-xs text-sky-800 dark:text-sky-200 space-y-2 list-disc list-inside">
                            <li>Data ini digunakan untuk dropdown pilihan pejabat saat menerbitkan <strong>SPK</strong>
                                atau <strong>Surat Pengantar</strong>.</li>
                            <li>Pastikan <strong>NIP</strong> dan <strong>Jabatan</strong> sesuai untuk keperluan surat
                                resmi.</li>
                        </ul>
                    </div>
                </div>
    ...
    {{-- MODAL FORM --}}
    <flux:modal name="signatory-form" class="min-w-[30rem]" :show="$showForm">
        <div class="space-y-6">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <flux:heading size="lg">{{ $editingId ? 'Edit Pejabat' : 'Tambah Pejabat' }}</flux:heading>
                    <flux:subheading class="mt-1">Isi data pejabat penandatangan.</flux:subheading>
                </div>
            ...
            <div class="grid gap-4">
                <flux:input label="Nama Lengkap (dengan gelar)" wire:model.defer="name" :invalid="$errors->has('name')"
                    placeholder="Dr. Ir. Fulan, M.T." />

                <flux:input label="NIP" wire:model.defer="nip" :invalid="$errors->has('nip')" />

                <flux:input label="Jabatan" wire:model.defer="position" :invalid="$errors->has('position')"
                    placeholder="Wakil Dekan Bidang Akademik" />
            </div>
    ...
    <flux:modal name="delete-confirm" class="min-w-[24rem]">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg" class="text-rose-600">Hapus Penandatangan?</flux:heading>
