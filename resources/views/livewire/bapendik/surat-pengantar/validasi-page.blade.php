<div class="space-y-6">
            ...
            <flux:card>
                <div class="flex items-center justify-between mb-3 gap-2">
                    <div class="flex items-center gap-2 w-full md:w-96">
                        <flux:input icon="magnifying-glass" placeholder="Cari perusahaan, penerima, nomor, nama/NIM…"
                ...
                <flux:table :paginate="$this->ordersPending">
                    <flux:table.columns>
                        ...
                        <flux:table.column sortable :sorted="$sortBy === 'tanggal_pengajuan_surat_pengantar'"
                            :direction="$sortDirection" wire:click="sort('tanggal_pengajuan_surat_pengantar')">Tanggal
                        </flux:table.column>
                        ...
                        @foreach ($this->ordersPending as $idx => $row)
                            <flux:table.row :key="'p-'.$row->id">
                                ...
                                <flux:table.cell class="whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-zinc-500">NIM: {{ $row->mahasiswa?->mahasiswa_nim ?: '—' }}
                                    </div>
                                </flux:table.cell>
                ...
                <flux:table :paginate="$this->ordersPublished">
                    <flux:table.columns>
                        ...
                        <flux:table.column sortable :sorted="$sortBy === 'tanggal_disetujui_surat_pengantar'"
                            :direction="$sortDirection" wire:click="sort('tanggal_disetujui_surat_pengantar')">Tgl
                            Terbit</flux:table.column>
                        <flux:table.column>Mahasiswa</flux:table.column>
                    ...
                    <flux:table.rows>
                        @foreach ($this->ordersPublished as $idx => $row)
                            <flux:table.row :key="'pb-'.$row->id">
                                ...
                                <flux:table.cell class="whitespace-nowrap">
                                    {{ $row->mahasiswa?->mahasiswa_name ?: '—' }}
                                    <div class="text-xs text-zinc-500">NIM:
                                        {{ $row->mahasiswa?->mahasiswa_nim ?: '—' }}</div>
                                </flux:table.cell>

                                <flux:table.cell class="whitespace-nowrap">{{ $row->nomor_surat ?: '—' }}
                                </flux:table.cell>
                                <flux:table.cell class="whitespace-nowrap">{{ $row->lokasi_surat_pengantar }}
                                </flux:table.cell>
                                <flux:table.cell>
                                    ...
                                    <flux:dropdown position="bottom" align="end">
                                        ...
                                        <flux:menu class="min-w-52">
                                            <flux:menu.item icon="arrow-down-tray"
                                                href="{{ route('bap.sp.download.docx', $row) }}" target="_blank">
                                                Unduh DOCX
                                            </flux:menu.item>
    ...
    <flux:modal name="sp-reject" class="min-w-[28rem]">
        <div class="space-y-6">
            ...
            <flux:textarea label="Catatan penolakan" wire:model.defer="catatan_tolak" placeholder="Alasan penolakan..."
                rows="4" />
    ...
    <flux:modal name="sp-publish" class="min-w-[32rem]">
        <div class="space-y-6">
            <div>
                ...
                <flux:subheading class="mt-1">Masukkan nomor surat dan pastikan penandatangan benar.
                </flux:subheading>
            </div>
